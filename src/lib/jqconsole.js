...
(function() {
		...
		Ansi.prototype._style = function(code) {
			...
			switch (code) {
				...
				case 47:
					...
					return this._append(
						"background-color-" + this._color(code - 40)
					);
				case 49:
	...
	JQConsole = (function() {
		function JQConsole(
			...
			this.$console = $('<pre class="jqconsole"/>').appendTo(
				this.$container
			);
			this.$console.css({
				margin: 0,
		...
		JQConsole.prototype.RegisterShortcut = function(key_code, callback) {
			...
			if (typeof callback !== "function") {
				throw new Error(
					"Callback must be a function, not " + callback + "."
				);
			}
		...
		JQConsole.prototype.ClearPromptText = function(clear_label) {
			if (this.state === STATE_OUTPUT) {
				throw new Error(
					"ClearPromptText() is not allowed in output state."
				);
			}
		...
		JQConsole.prototype.GetPromptText = function(full) {
			...
			if (this.state === STATE_OUTPUT) {
				throw new Error(
					"GetPromptText() is not allowed in output state."
				);
			}
			if (full) {
				this.$prompt_cursor.text("");
		...
		JQConsole.prototype.SetPromptText = function(text) {
			if (this.state === STATE_OUTPUT) {
				throw new Error(
					"SetPromptText() is not allowed in output state."
				);
			}
		...
		JQConsole.prototype.SetPromptLabel = function(
			main_label,
			continue_label
		) {
		...
		JQConsole.prototype.Prompt = function(
			...
			this.$prompt.attr(
				"class",
				CLASS_PROMPT + " " + this.ansi.getClasses()
			);
			this.$prompt_label.text(this._SelectPromptLabel(false));
			if (this.auto_focus) {
				this.Focus();
		...
		JQConsole.prototype.UnRegisterMatching = function(open, close) {
			...
			return this.matchings.clss.splice(
				this.matchings.clss.indexOf(cls),
				1
			);
		};
		...
		JQConsole.prototype.Clear = function() {
			var prompt_class;
			prompt_class =
				this.state === STATE_INPUT ? CLASS_INPUT : CLASS_PROMPT;
			this.$console
				.find("." + CLASS_HEADER)
					...
					return function() {
						...
						removeClass = function() {
							if (_this.$console_focused) {
								return _this.$console.removeClass(
									CLASS_BLURRED
								);
							}
						};
		...
		JQConsole.prototype._HandleKey = function(event) {
			...
			if (this.custom_control_key_handler != null) {
				if (
					this.custom_control_key_handler.call(this, event) === false
				) {
					return false;
				}
			}
						...
						if (indent !== false) {
							...
							if (_this.history_active) {
								if (
									...
									_this.history[_this.history.length - 1] !==
										text
								) {
		...
		JQConsole.prototype._VerticalMove = function(up) {
			...
			return $prompt_which.text(
				up ? text.slice(0, pos) : text.slice(pos)
			);
		};
			...
			if (text) {
				if (whole_word) {
					...
					$prompt_opposite.text(
						back ? text.slice(-1) + tmp : tmp + text[0]
					);
					return $prompt_which.text(
						back ? text.slice(0, -1) : text.slice(1)
					);
				}
			} else if (!$prompt_relative.is(EMPTY_SELECTOR)) {
				$which_line = $(EMPTY_SPAN)[where_append]($prompt_rel_opposite);
				$which_line.append(
					$(EMPTY_SPAN).text(this.$prompt_label.text())
				);
		...
		JQConsole.prototype._MoveTo = function(all_lines, back) {
			...
			if (all_lines) {
				...
				while (
					!(
						$prompt_relative.is(EMPTY_SELECTOR) &&
						$prompt_which.text() === ""
					)
				) {
					MoveToLimit(false);
			...
			if (
				...
				if (screen_top + doc_height < pos.top) {
					return this.$window.scrollTop(
						pos.top - doc_height + line_height
					);
		...
		JQConsole.prototype._ProcessMatch = function(
			config,
			back,
			before_char
		) {
		...
		JQConsole.prototype._CheckMatchings = function(before_char) {
			...
			current_char = before_char
				? this.$prompt_left
						.text()
						.slice(this.$prompt_left.text().length - 1)
